// 0. ATRAPAMOS LA PALABRA SECRETA DE DJANGO
    const targetWord = JSON.parse(document.getElementById('palabra-secreta-data').textContent).toUpperCase();
    ...
    const prioridadColor = { correct: 3, present: 2, absent: 1 };
    const estadoTeclado = {};
    ...
    function showPlayOverlay() {
        ...
        overlay.addEventListener('click', (e) => {
            ...
            btn.disabled = true;
            videoSecreto.play().then(() => { overlay.remove(); }).catch(err => { btn.disabled = false; });
        });
        document.body.appendChild(overlay);
    }
    videoSecreto.onended = () => { videoSecreto.style.display = 'none'; };

    // 3. LÓGICA CENTRAL DEL JUEGO (MODIFICADA PARA LOCAL)
    function manejarEntrada(letra) {
        ...
        if (letra === "DEL") {
            ...
            if (intentoActual.length === 5) {
                ...
                // LÓGICA DE COMPROBACIÓN LOCAL DE COLORES
                let colors = Array(5).fill('absent');
                let targetArr = targetWord.split('');
                let guessArr = intentoActual.split('');
                let win = true;
                ...
                for (let i = 0; i < 5; i++) {
                    if (guessArr[i] === targetArr[i]) {
                        colors[i] = 'correct';
                        targetArr[i] = null; // Marcamos como usada
                        guessArr[i] = null;
                    } else {
                        win = false;
                    }
                ...
                for (let i = 0; i < 5; i++) {
                    if (guessArr[i] !== null) {
                        ...
                        if (index !== -1) {
                            colors[i] = 'present';
                            targetArr[index] = null; // Marcamos como usada
                        }
                    }
                }
                    ...
                    if (win) {
                        ...
                        mostrarModalFinal("¡OH NO!", `
                            ...
                            <div class="bg-slate-50 dark:bg-slate-700 border-2 border-slate-200 dark:border-slate-600 p-3 rounded-xl mb-4">
                                ...
                                <p class="text-3xl font-extrabold text-primary font-display">${targetWord}</p>
                            </div>
